...
for sales_col in sales_columns:
    ...
    # Define model
    model = ElasticNet(l1_ratio=0.000000000055)
    ...
    grid = dict()
    grid['l1_ratio']=np.linspace(0.00000000005,0.00000000006,9)
    ...
    model = ElasticNet(l1_ratio=results.best_params_['l1_ratio'])
    model.fit(X_train, y_train)
